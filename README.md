...
프리셋 항목 오른쪽의 연필 버튼으로 수정하고, X 버튼으로 삭제합니다.

수정 대화창은 저장된 값이 모두 채워진 채로 열리므로 옵션 일부만 고쳐 저장할 수 있습니다. **Preset Name**을 바꾸면 이름 바꾸기가 되어 예전 이름의 프리셋은 사라집니다. 이미 있는 다른 프리셋의 이름으로 바꾸면 그 프리셋을 덮어씁니다.

### 파일 이름 규칙

파일 이름은 오브젝트 이름(`Single File` 모드는 액티브 오브젝트 이름)을 그대로 씁니다. 다만 파일 이름에 쓸 수 없는 문자(`\ / : * ? " < > |`)는 `_`로 바꾸고, 이름 끝의 공백은 지웁니다. 이름이 바뀌면 경고로 알려 줍니다. `Body:Arm` → `Body_Arm.fbx`.

내보내기에 실패한 오브젝트가 있으면 몇 개가 왜 실패했는지 알려 주고, 하나도 내보내지 못하면 오류로 처리합니다.
