...
- 내보내기 프리셋 수정 기능을 추가했습니다. 하위 메뉴의 프리셋 오른쪽 연필 버튼을 누르면 저장된 값이 그대로 채워진 대화창이 열리고, 옵션과 이름을 고쳐 저장할 수 있습니다. 이름을 바꾸면 예전 이름의 프리셋은 사라집니다(이름 바꾸기).
- 오브젝트 이름에 파일 이름으로 쓸 수 없는 문자(`\ / : * ? " < > |`)가 있으면 `_`로 바꿔 내보냅니다. 그동안 `Body:Arm` 같은 이름은 오류 없이 엉뚱한 파일이 생기고 `.fbx`는 만들어지지 않았으며, `Body/Arm`처럼 경로 구분 문자가 들어가면 내보내기가 아예 실패했습니다.
- 내보내기 결과를 실제로 확인합니다. 연산자가 취소되거나 파일이 만들어지지 않으면 실패로 세고, 하나도 내보내지 못하면 성공 메시지 대신 오류를 냅니다. 일부만 실패하면 몇 개가 왜 실패했는지 알려 줍니다.
- 내보내다 실패해도 실행 전 선택 상태와 액티브 오브젝트를 반드시 복원합니다.
